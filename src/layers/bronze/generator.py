...
# 1. Define the path to the folder containing c.py
current_dir = os.path.dirname(os.path.abspath(__file__))
config_dir = os.path.abspath(os.path.join(current_dir, "../../../config"))
# 2. Add that folder to the system path
sys.path.append(config_dir)
# 3. Now you can import it like a normal module
try:
    import config as c
    import paths as p # type: ignore
    from logger_setup import setup_logging # type: ignore
except ImportError as e:
    logging.critical(f"Failed to import project modules. Ensure c.py, logger_setup.py and file_selector.py are accessible: {e}")
    sys.exit(1)


sys.path.append(p.SRC_UTILS_DIR)
try:
...
def get_config_from_filename(filename: str) -> Tuple[Optional[Dict], Optional[float]]:
    """Parses a filename to extract instrument and timeframe, returning the c."""
    ...
    spread_in_pips = c.SIMULATION_SPREAD_PIPS.get(instrument.upper(), c.SIMULATION_SPREAD_PIPS["DEFAULT"])
    ...
    return c.TIMEFRAME_PRESETS[timeframe_key], spread_cost
...
def main() -> None:
    ...
    logger.info(f"Using up to {c.MAX_CPU_USAGE} CPU cores for simulation.")
    ...
    if target_file_arg:
        ...
        if not os.path.exists(os.path.join(p.RAW_DATA_DIR, target_file_arg)):
            logger.error(f"Target file not found: {os.path.join(p.RAW_DATA_DIR, target_file_arg)}")
        else:
