    ...
    for level in all_levels:
        for sltp in ['sl', 'tp']:
            ...
            if bps_col in chunk.columns:
                bin_size = c.PLATINUM_BPS_BIN_SIZE
...
def flush_buffer_to_shards(buffer: List[Tuple], temp_dir: str):
    ...
    for shard_id, group in df.groupby('shard_id'):
        filepath = os.path.join(temp_dir, f"{c.PLATINUM_TEMP_SHARD_PREFIX}{shard_id}.parquet")
...
def run_preprocessor_for_instrument(instrument_name: str, base_dirs: Dict[str, str]) -> None:
    ...
    chunked_outcomes_dir = os.path.join(base_dirs['silver_chunked_outcomes'], instrument_name)
    ...
    final_targets_dir = os.path.join(base_dirs['platinum_targets'], instrument_name)
...
def main() -> None:
    ...
    setup_logging(p.LOGS_DIR, c.CONSOLE_LOG_LEVEL, c.FILE_LOG_LEVEL, "platinum_prepper_layer")

    start_time = time.time()
    base_dirs = {
        'silver_chunked_outcomes': p.SILVER_DATA_CHUNKED_OUTCOMES_DIR,
        'platinum_combo': p.PLATINUM_DATA_COMBINATIONS_DIR,
        'platinum_temp': p.PLATINUM_DATA_TEMP_DIR,
    ...
    if target_file_arg:
        ...
        if os.path.exists(target_path):
            folders_to_process = [target_file_arg]
        ...
        folders_to_process = select_files_interactively(new_files)

    if not folders_to_process:
        logger.info("No files selected. Exiting.")
        return
    
    logger.info(f"Queued {len(folders_to_process)} folders(s): {', '.join(folders_to_process)}")
    for folder in folders_to_process:
        logger.info(f"--- Processing Instrument: {folder} ---")
        run_preprocessor_for_instrument(folder, base_dirs)
